...
exports.start = (app, module, done) =>
{
  ...
  module.setInputMode = function(inputMode, done)
  {
    ...
    module.debug("Changed input mode.", {
      oldInputMode: module.currentState.inputMode,
  ...
  module.setWorkMode = function(workMode, done)
  {
    ...
    module.debug("Changed work mode.", {
      oldWorkMode: module.currentState.workMode,
  ...
  module.selectOrderNo = function(orderNo, done)
  {
    ...
    module.debug("Changed selected order.", {
      oldOrderNo: module.currentState.selectedOrderNo,
      newOrderNo: orderNo
    });
  ...
  module.selectNc12 = function(nc12, password, done)
  {
    ...
    module.debug("Changed selected 12NC", {
      oldCode: module.currentState.selectedNc12,
      newCode: nc12
    });
  ...
  module.setProgram = function(programId, done)
  {
    ...
    sqlite3Module.db.get("SELECT * FROM programs WHERE _id=?", [programId], function(err, program)
    {
      ...
      module.debug("Changed selected program.", {
        oldProgramId: module.currentState.program ? module.currentState.program._id : null,
  ...
  module.reload = function(done)
  {
    ...
    sqlite3Module.db.get(sql, function(err, row)
    {
      ...
      module.debug("Reloaded last order.", {orderId: row ? row._id : null});
  ...
  module.toggleResult = function(resultId, state, done)
  {
    ...
    module.info(`${newCancelled ? "Cancelling" : "Restoring"} result.`, {resultId});
    ...
    step(
      ...
      function finalizeStep(err)
      {
        if (err)
        {
          module.error(err, "Failed to toggle result.", {resultId});
        }
        else
        {
          module.info("Toggled result.", {resultId});
  ...
  function readLastMode(done)
  {
    (safeFs || fs).readFile(module.config.lastModeFile, {encoding: 'utf8'}, function(err, lastMode)
    {
      if (err)
      {
        module.warn(err, "Failed to read the last mode file.");
      }
      else
      {
        try
        ...
        {
          module.warn(err, "Failed to parse the last mode file.");
        }
      }
  ...
  function saveLastMode()
  {
    ...
    (safeFs || fs).writeFile(module.config.lastModeFile, JSON.stringify(lastMode), function(err)
    {
      if (err)
      {
        module.error(err, "Failed to save the last mode.");
      }
  ...
  function updateWorkMode()
  {
    ...
    module.setWorkMode(newWorkMode, function(err)
    {
      if (err)
      {
        module.error(err, "Failed to switch the work mode after settings were changed.", {newWorkMode});
      }
    });
  }
