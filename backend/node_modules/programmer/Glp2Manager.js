...
function Glp2Manager(app, programmer)
{
   ...
   * @type {Object}
   */
  this.logger = programmer.logger.create({submodule: 'glp2'});
...
Glp2Manager.prototype.start = function(done)
{
  ...
  step(
    ...
    function createMasterStep(err, comPort) // eslint-disable-line handle-callback-err
    {
...
Glp2Manager.prototype.reset = function(cancelDelay, done)
{
  ...
  step(
    ...
    function delayNextStep(err)
    {
      if (err)
      {
        manager.logger.error(err, "Failed to cancel test.");
    ...
    function removeTestProgramsStep(err)
    {
      if (err)
      {
        manager.logger.error(err, "Failed to remove test results.");
    ...
    function setParametersStep(err)
    {
      if (err)
      {
        manager.logger.error(err, "Failed to remove test programs.");
    ...
    function handleResponseStep(err)
    {
      if (err)
      {
        manager.logger.error(err, "Failed to set parameters.");
...
Glp2Manager.prototype.checkReadyState = function()
{
  ...
  this.start(function(err)
  {
    if (err)
    {
      manager.logger.error(err, "Failed to restart.");
    }
  });
};
...
Glp2Manager.prototype.monitorActualValues = function(timeouts)
{
  ...
  this.master.getActualValues(function(err, res)
  {
    ...
    if (err)
    {
      ...
      if (timeouts < 5)
      {
        manager.logger.error(err, "Failed to monitor actual values.");
      }
...
Glp2Manager.prototype.onAppStarted = function()
{
  ...
  this.start(function(err)
  {
    ...
    {
      manager.logger.debug("Failed to start after app started.");
    }
  });
};
...
Glp2Manager.prototype.onSettingsChanged = function(changes)
{
  ...
  this.stop(function(err)
  {
    if (err)
    {
      return manager.logger.error(err, "Failed to stop after settings changed.");
    }
    ...
    manager.start(function(err)
    {
      if (err)
      {
        return manager.logger.error(err, "Failed to start after settings change.");
      }
    });
  });
};
...
Glp2Manager.prototype.onProgrammerFinished = function()
{
  ...
  this.reset(function(err)
  {
    if (err)
    {
      manager.logger.error(err, "Failed to reset the tester after programming.");
    }
  });
};
...
Glp2Manager.prototype.onMasterOpen = function()
{
  ...
  this.reset(function(err)
  {
    if (err)
    {
      manager.logger.error(err, "Failed to reset the tester after connecting.");
    }
    else
    {
...
Glp2Manager.prototype.getDeviceOptions = function()
{
  ...
  this.master.getDeviceOptions(function(err, res)
  {
    if (err)
    {
      return manager.logger.error(err, "Failed to get device options.");
    }

    manager.deviceOptions = res.deviceOptions;
    manager.softwareVersion = res.getSoftwareVersion();

    manager.logger.info("Device options.", {deviceOptions: manager.deviceOptions});
  });
};
