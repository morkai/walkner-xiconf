  ...
  step(
    ...
    function tryToProgramStep()
    {
      ...
      function updateProgress()
      {
        ...
        while ((pos = progress.indexOf('Progress: ')) !== -1) // eslint-disable-line no-cond-assign
        {
    ...
    function writeOutputFileStep(err, outputFileContents)
    {
      ...
      fs.unlink(programmerModule.config.gprsOutputFile, function(err)
      {
        if (err)
        {
          programmerModule.error(err, "Failed to remove the GPRS output file.", {submodule: 'gprs'});
        }
      });
