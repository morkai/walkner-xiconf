...
function RemoteCoordinator(app, programmerModule)
{
  ...
  this.logger = programmerModule.logger.create({submodule: 'remote'});
  this.sio = null;
...
RemoteCoordinator.prototype.setUpSio = function()
{
  ...
  {
    rc.logger.debug("Connecting...");
  });

  sio.once('connect', function()
  {
    rc.logger.debug("Connected.", {remoteServer});
  ...
  {
    rc.logger.debug(err, "Failed to connect.");
  });

  sio.on('disconnect', function()
  {
    rc.logger.warn("Disconnected :(");
  ...
  sio.on('reconnecting', function(reconnectCount)
  {
    if (reconnectCount === 1)
    {
      rc.logger.debug("Reconnecting...");

      sio.once('reconnect_error', function(err)
      {
        rc.logger.debug(err, "Failed to reconnect.");
      });
    }
  });
  ...
  sio.on('reconnect', function()
  {
    ...
    rc.logger.debug("Reconnected.", {remoteServer});
...
RemoteCoordinator.prototype.request = function(action, body, done, rid, attempt, cancel)
{
  ...
  cancel.req = request(options, function(err, res, responseBody)
  {
    ...
    if (err)
    {
      ...
      rc.logger.debug("Acting...", {attempt: attempt + 1, action});
...
{
  this.logger.debug('Leader updated.', {newLeader});
...
{
  this.logger.debug('Programs update requested...');
...
RemoteCoordinator.prototype.onRestart = function()
{
  ...
  if (this.programmer.currentState.isInProgress())
  {
    this.logger.debug('Restart requested while programming...');
...
{
  this.logger.debug('Update check requested...');
...
RemoteCoordinator.prototype.onConfigure = function(settings, reply)
{
  if (this.programmer.currentState.isInProgress())
  {
    this.logger.debug('Settings change requested while programming...');

    this.broker.subscribe('programmer.finished', this.onConfigure.bind(this, settings)).setLimit(1);
  }
  else
  {
    this.logger.debug('Importing settings.', {settings});
...
RemoteCoordinator.prototype.onLedUpdated = function(message)
{
  ...
  this.logger.debug('Recording invalid LED.', {invalidLed: data});
...
{
  this.logger.debug('No reconnect after 15s. Resetting remote data...');
...
RemoteCoordinator.prototype.checkLastResponseTime = function()
{
  ...
  {
    this.logger.debug('No activity for 45s. Forcing reconnect...');
