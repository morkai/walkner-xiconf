...
module.exports = function program(app, programmerModule, data, done)
{
  ...
  if (currentState.inputMode === 'remote' && !remoteCoordinator.isConnected())
  {
  ...
  function waitForLedsStep()
  {
    ...
    function addLedsFromLastResult()
    {
      history.findLedsFromRecentFailure(currentState.order.no, currentState.nc12, function(err, leds)
      {
        ...
        if (err)
        {
          return programmerModule.error(err, "Failed to find LEDs from recent failure.");
        }
    ...
    remoteCoordinator.acquireServiceTag(serviceTagRequestData, function(err, serviceTag)
    {
      if (thisProgrammingCancelled)
      {
        if (serviceTag)
        {
          ...
          remoteCoordinator.releaseServiceTag(serviceTagRequestData, function(err)
          {
            programmerModule.warn(err, 'Failed to release service tag.', {serviceTagRequestData});
          });
        }
  ...
  function finalizeStep(err)
  {
    ...
    currentState.save(programmerModule.config.featureDbPath, function(err)
    {
      if (err)
      {
        programmerModule.error(err, "Failed to save the current state.");
      }
