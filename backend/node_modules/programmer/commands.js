...
{
  const logger = programmerModule.logger.create({submodule: 'sio'});
  const settings = app[programmerModule.config.settingsId];
  const sio = app[programmerModule.config.sioId];

  let localSocketCounter = 0;
  ...
  sio.on('connection', function(socket)
  {
    ...
    logger.debug("Client connected.", {isLocal, remoteAddress});
    ...
    {
      logger.debug("Client disconnected.", {isLocal, remoteAddress});
    });
  });
  ...
  function printServiceTags(orderNo, items, reply)
  {
    ...
    while ((matches = re.exec(items)) !== null) // eslint-disable-line no-cond-assign
    {
