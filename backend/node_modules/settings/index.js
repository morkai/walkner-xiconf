...
exports.start = function startSettingsModule(app, module, done)
{
  ...
  app.broker.subscribe('settings.changed')
    ...
    .on('message', function()
    {
      readMultiOneWorkflowVersion(function(err, multiOneWorkflowVersion) // eslint-disable-line handle-callback-err
      {
        module.import({multiOneWorkflowVersion: multiOneWorkflowVersion || '0.0.0.0'}, () => {}, false, true);
      });
    });
  ...
  function onAppStarted()
  {
    ...
    step(
      ...
      function(err, multiOneWorkflowVersion, coreScannerDriver, remoteSettings) // eslint-disable-line handle-callback-err
      {
  ...
  function readInitialSettings(done)
  {
    ...
    (safeFs || fs).readFile(module.config.settingsFile, {encoding: 'utf8'}, function(err, contents)
    {
      ...
      {
        module.warn(err, "Failed to parse the settings file.");
      }
  ...
  {
    var programmerFile = module.get('programmerFile');

    if (process.platform !== 'win32' || !programmerFile || programmerFile.includes('fake'))
    {
      return done(null, null);
    }

    var cmd = format('"%s" /f dummy /w dummy /c Halt', programmerFile);

    module.debug("Checking MultiOne Workflow version...");

    exec(cmd, {timeout: 30000}, function(err, stdout) // eslint-disable-line handle-callback-err
    {
      var matches = stdout.match(/v(?:ersion)?.*?((?:[0-9]+\.?){4})/);
      var multiOneWorkflowVersion = matches ? matches[1] : '0.0.0.0';

      module.debug("MultiOne Workflow version checked.", {multiOneWorkflowVersion});
  ...
  function checkCoreScannerDriver(done)
  {
    ...
    {
      return fs.stat('/usr/bin/cscore', (err, stats) =>
      {
        const coreScannerDriver = !!stats;

        module.debug("CoreScanner driver availability checked.", {coreScannerDriver});
    ...
    exec('sc qc CoreScanner', {timeout: 30000}, (err, stdout) => // eslint-disable-line handle-callback-err
    {
      const coreScannerDriver = _.includes(stdout, 'SERVICE_NAME: CoreScanner');
  ...
  function updateRemoteSettings(save, done)
  {
    step(
      ...
      function(err, remoteSettings) // eslint-disable-line handle-callback-err
      {
  ...
  function updateNextRemoteSettings(remoteSettingsUrls, done)
  {
    ...
    module.debug("Updating remote settings...", {remoteSettingsUrl: url});
