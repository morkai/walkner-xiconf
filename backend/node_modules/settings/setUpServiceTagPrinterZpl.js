...
module.exports = function setUpServiceTagPrinterZpl(app, settings)
{
  ...
  step(
    ...
    function findPrinterPortsStep(err, stdout)
    {
      ...
      while ((matches = re.exec(stdout)) !== null) // eslint-disable-line no-cond-assign
      {
    ...
    function addPrinterPortsStep()
    {
      ...
      {
        settings.debug("Adding a new printer...", {printerPort: this.ports[i]});
    ...
    function finalizeStep(err)
    {
      if (err)
      {
        settings.error(err, "Failed to set up the Service Tag printer.");
      }
      else
      {
