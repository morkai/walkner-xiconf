...
const setUpRemoteExport = require('./remoteExport');
...
const MAX_RECENT_SN_HISTORY_ENTRIES = 25;

exports.start = (app, module, done) =>
{
  const sqlite3 = app[module.config.sqlite3Id];
  ...
  {
    return new HistoryEntry(sqlite3.db, app.broker.sandbox(), app[module.config.settingsId]);
  };
  ...
  module.findLedsFromRecentFailure = function(orderNo, nc12, done)
  {
    ...
    sqlite3.db.get(sql, {}, function(err, row)
    {
      if (err)
      {
        return done(err);
  ...
  setUpDb(sqlite3, done);

  function findRecentEntries()
  {
    const sql = `
      SELECT e.*, o._id AS orderId, o.no AS orderNo, o.quantity as orderQuantity
    ...
    sqlite3.db.all(sql, function(err, historyEntries)
    {
      if (err)
      {
        return module.error(err, "Failed to find recent entries.");
      }
