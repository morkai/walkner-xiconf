...
{
  const db = app[module.config.sqlite3Id].db;
  const settings = app[module.config.settingsId];
  const safeFs = app[module.config.safeFsId];
  ...
  function syncNow()
  {
    if (!isValidLicense())
    {
      module.warn("Stopping the remote export: invalid license.");
  ...
  function readLastExportTimeStep(err)
  {
    ...
    (safeFs || fs).readFile(module.config.lastExportTimeFile, 'utf8', function(err, contents)
    {
  ...
  function initArchiveStep(err)
  {
    ...
    {
      cachedEncryptedUuids[uuid] = crypto.publicEncrypt(settings.config.licenseEdPem, Buffer.from(uuid, 'utf8')).toString('base64');
    }
  ...
  function packFeatureFilesStep(err)
  {
    ...
    step(
      function()
      {
        ...
        {
          var featureFilePath = path.join(module.config.featureDbPath, featureFileHash);
          var next = ctx.parallel();

          fs.readFile(featureFilePath, function(err, buf) // eslint-disable-line handle-callback-err
          {
            if (buf)
            {
  ...
  function saveLastExportTimeStep(err)
  {
    ...
    var file = module.config.lastExportTimeFile;
  ...
  function finalizeStep(err)
  {
    ...
    if (err)
    {
      module.error(err, "Failed to export data to the remote server.");
    }
    else
    {
      var licenseInfo = settings.get('licenseInfo');
