...
COMMIT TRANSACTION;`;
  ...
  step(
    function()
    {
      module.db.exec(baseSchemaSql, this.next());
    },
    function(err)
    {
      if (err)
      ...
      module.db.get('PRAGMA user_version', this.next());
    },
    function(err, row)
    {
      if (err)
      ...
      if (sql === '')
      {
        return module.info(`Not updating the database.`, {version: userVersion});
      }

      module.info(`Updating the database...`, {
        oldVersion: row.user_version,
        newVersion: userVersion
      });

      sql = 'BEGIN TRANSACTION;\n' + sql + 'PRAGMA user_version=' + userVersion + ';\nCOMMIT TRANSACTION;';

      module.db.exec(sql, this.next());
    },
    done
  );
};
